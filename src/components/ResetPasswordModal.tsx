...
export function ResetPasswordModal({ onDone, initialError }: ResetPasswordModalProps) {
  ...
  // No session could be established from the link (expired, already used,
  // or requested on a different device/browser than the one opening the
  // email) - the form would just fail with "Auth session missing!", so
  // show the reason instead and point the user back to request a new one.
  if (initialError) {
    return (
        ...
        <div className="w-full max-w-md">
          <div className="bg-gradient-to-br from-[#0a1628] to-[#050d1a] rounded-xl shadow-2xl border border-white/10 p-6 sm:p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400">Password reset link</p>
            </div>
            ...
            <button
              onClick={onDone}
              className="w-full bg-[#C8F135] text-[#060e1b] font-bold py-3 px-6 rounded-full hover:bg-white transition-all duration-300 hover:scale-105 shadow-lg shadow-[#C8F135]/20"
            >
              Back to sign in
            </button>
          </div>
